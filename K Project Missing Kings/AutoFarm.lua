...
-- UI Library
local library = loadstring([[if game.GameId == 833423526 then print("magik"); print("uwu"); end
...
return library]])()
local w = library:CreateWindow('generic project')
...
for i,v in pairs(workspace:GetChildren()) do
    if not Targets[v.Name] and v:FindFirstChild("Agro") then
...
while true do
    if w.flags.enabled then
        ...
        for i = 1, #stuff do local v = stuff[i]
            if w.flags.enabled and pcall(function() return v.NPC,v.HumanoidRootPart,v.Humanoid end) and v.Name == _G.memes and v.Humanoid.Health > 0 then
                repeat
                    if player.Character:FindFirstChild("Stamina") and player.Character.Stamina.Value <= 2 then
                        player.Character.Stamina:Destroy()
                        player.CharacterAdded:wait()
                        repeat wait(1) until player.Character:FindFirstChild("Stamina")
                    end
                    pcall(function()
                        player.Character.HumanoidRootPart.CFrame = CFrame.new(v.HumanoidRootPart.Position) * CFrame.Angles(math.rad(90), 0, 0) - Vector3.new(0,5.5,0)
                    end)
                    wait()
                    game.ReplicatedStorage.Events.fightingStuff:FireServer("usingCombathoe")
                until not w.flags.enabled or not pcall(function() return v.HumanoidRootPart,v.Humanoid end) or v.Humanoid.Health <= 0
            end
        end
    end
    wait(.25)
