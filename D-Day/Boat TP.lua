...
library = library()
local w = library:CreateWindow('boat tp')
local flags = {}
local player = game.Players.LocalPlayer
for i,v in pairs(workspace.Scriptables.Flags:GetChildren()) do
...
w:Dropdown("locations", {
    ...
    for i = 1, #stuff do local v = stuff[i]
        if v.Name == "Vehicle" and pcall(function() return v.VehicleSeat.SeatWeld end) and v.VehicleSeat.SeatWeld.Part1 == player.Character.HumanoidRootPart then
            for i2,v2 in pairs(v:GetDescendants()) do
                if pcall(function() return v2.CFrame end) then
                    v2.CFrame = workspace.Scriptables.Flags[new].Flag.CFrame
                end
            end
        end
    end
